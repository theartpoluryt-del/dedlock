...
void UpdatePanoramaPreview(IDXGISwapChain* swapChain,
    ...
    if (!visible) {
        panelVisible.store(false, std::memory_order_release);
        // A window-message dispatch can arrive while Panorama is rebuilding
        // its context. Retry until the hide generation is actually applied.
        if (requestPending && gameWindow && now - lastUiDispatchAt >= 16) {
